;;; tl-mode.el --- a major-mode for editing TypedLua scripts
...
;;
;; URL:         http://immerrr.github.com/tl-mode
;; Version:     20151025
;;
...
;; tl-mode provides support for editing Lua, including automatical
...
;; - Var `tl-indent-string-contents':
;;   set to `t` if you like to have contents of multiline strings to be
;;   indented like comments
;; - Var `tl-mode-hook':
;;   list of functions to execute when tl-mode is initialized
;; - Var `tl-documentation-url':
;;   base URL for documentation lookup
;; - Var `tl-documentation-function': function used to
;;   show documentation (`eww` is a viable alternative for Emacs 25)

;; These are variables/commands that operate on the Lua process:

;; - Var `tl-default-application':
;;   command to start the Lua process (REPL)
;; - Var `tl-default-command-switches':
;;   arguments to pass to the Lua process on startup (make sure `-i` is there
;;   if you expect working with Lua shell interactively)
;; - Cmd `tl-start-process': start new REPL process, usually happens automatically
;; - Cmd `tl-kill-process': kill current REPL process

;; These are variables/commands for interaction with the Lua process:

;; - Cmd `tl-show-process-buffer': switch to REPL buffer
;; - Cmd `tl-hide-process-buffer': hide window showing REPL buffer
;; - Var `tl-always-show': show REPL buffer after sending something
;; - Cmd `tl-send-buffer': send whole buffer
;; - Cmd `tl-send-current-line': send current line
;; - Cmd `tl-send-defun': send current top-level function
;; - Cmd `tl-send-region': send active region
;; - Cmd `tl-restart-with-whole-file': restart REPL and send whole buffer

;; See "M-x apropos-command ^tl-" for a list of commands.
...
(eval-and-compile
  (defvar tl-rx-constituents)
  (defvar rx-parent)

  (defun tl-rx-to-string (form &optional no-group)
    "Tl-specific replacement for `rx-to-string'.

See `rx-to-string' documentation for more information FORM and
NO-GROUP arguments."
    (let ((rx-constituents tl-rx-constituents))
      (rx-to-string form no-group)))

  (defmacro tl-rx (&rest regexps)
    "Tl-specific replacement for `rx'.

See `rx' documentation for more information about REGEXPS param."
    (cond ((null regexps)
           (error "No regexp"))
          ((cdr regexps)
           (tl-rx-to-string `(and ,@regexps) t))
          (t
           (tl-rx-to-string (car regexps) t))))

  (defun tl--new-rx-form (form)
    "Add FORM definition to `tl-rx' macro.
...
element is itself expanded with `tl-rx-to-string'. "
    (let ((name (car form))
          (form-definition (cdr form)))
      (when (and (listp form-definition) (eq ':rx (car form-definition)))
        (setcdr form (tl-rx-to-string (cadr form-definition) 'nogroup)))
      (push form tl-rx-constituents)))

  (defun tl--rx-symbol (form)
  ...
  (setq tl-rx-constituents (copy-sequence rx-constituents))

  (mapc #'tl--new-rx-form
        `((symbol tl--rx-symbol 1 nil)
          (ws . "[ \t]*") (ws+ . "[ \t]+")
          (tl-name :rx (symbol (regexp "[[:alpha:]_]+[[:alnum:]_]*")))
          (tl-funcname
           :rx (seq tl-name (* ws "." ws tl-name)
                    (opt ws ":" ws tl-name)))
          (tl-funcheader
           ;; Outer (seq ...) is here to shy-group the definition
           :rx (seq (or (seq (symbol "function") ws (group-n 1 tl-funcname))
                        (seq (group-n 1 tl-funcname) ws "=" ws
                             (symbol "function")))))
          (tl-number
          ...
          (tl-keyword
           :rx (symbol "and" "break" "do" "else" "elseif" "end"  "for" "function"
                       ...
                       "then" "until" "while" "typealias" "interface")))
        ))


;; Local variables
(defgroup tl nil
  "Major mode for editing TypedLua code."
  :prefix "tl-"
  :group 'languages)

(defcustom tl-indent-level 3
  "Amount by which TypedLua subexpressions are indented."
  :type 'integer
  :group 'tl
  :safe #'integerp)

(defcustom tl-comment-start "-- "
  "Default value of `comment-start'."
  :type 'string
  :group 'tl)

(defcustom tl-comment-start-skip "---*[ \t]*"
  "Default value of `comment-start-skip'."
  :type 'string
  :group 'tl)

(defcustom tl-default-application "lua"
  "Default application to run in Lua process."
  :type '(choice (string)
                 (cons string integer))
  :group 'tl)

(defcustom tl-default-command-switches (list "-i")
  "Command switches for `tl-default-application'.
Should be a list of strings."
  :type '(repeat string)
  :group 'tl)
(make-variable-buffer-local 'tl-default-command-switches)

(defcustom tl-always-show t
  "*Non-nil means display tl-process-buffer after sending a command."
  :type 'boolean
  :group 'tl)

(defcustom tl-documentation-function 'browse-url
...
(defcustom tl-documentation-url
...
(defvar tl-process nil
  "The active Lua process")

(defvar tl-process-buffer nil
  "Buffer used for communication with the Lua process")

(defun tl--customize-set-prefix-key (prefix-key-sym prefix-key-val)
  (cl-assert (eq prefix-key-sym 'tl-prefix-key))
  ...
  (if (fboundp 'tl-prefix-key-update-bindings)
      (tl-prefix-key-update-bindings)))

(defcustom tl-prefix-key "\C-c"
  "Prefix for all tl-mode commands."
  :type 'string
  :group 'tl
  :set 'tl--customize-set-prefix-key
  :get '(lambda (sym)
          (let ((val (eval sym))) (if val (single-key-description (eval sym)) ""))))

(defvar tl-mode-menu (make-sparse-keymap "TypedLua")
  "Keymap for tl-mode's menu.")

(defvar tl-prefix-mode-map
  (eval-when-compile
    (let ((result-map (make-sparse-keymap)))
      (mapc (lambda (key_defn)
              (define-key result-map (read-kbd-macro (car key_defn)) (cdr key_defn)))
            '(("C-l" . tl-send-buffer)
              ("C-f" . tl-search-documentation)))
      result-map))
  "Keymap that is used to define keys accessible by `tl-prefix-key'.

If the latter is nil, the keymap translates into `tl-mode-map' verbatim.")

(defvar tl--electric-indent-chars
...
(defvar tl-mode-map
  (let ((result-map (make-sparse-keymap))
    ...
    (unless (boundp 'electric-indent-chars)
      (mapc (lambda (electric-char)
              (define-key result-map
                ...
                #'tl-electric-match))
            tl--electric-indent-chars))
    (define-key result-map [menu-bar tl-mode] (cons "TypedLua" tl-mode-menu))
    ...
    (if (boundp 'tl-prefix-key)
        (define-key result-map (vector tl-prefix-key) tl-prefix-mode-map)
      (set-keymap-parent result-map tl-prefix-mode-map))
    result-map)
  "Keymap used in tl-mode buffers.")

(defvar tl-electric-flag t
  "If t, electric actions (like automatic reindentation) will happen when an electric
 key like `{' is pressed")
(make-variable-buffer-local 'tl-electric-flag)

(defcustom tl-prompt-regexp "[^\n]*\\(>[\t ]+\\)+$"
  "Regexp which matches the Lua program's prompt."
  :type  'regexp
  :group 'tl)

(defcustom tl-traceback-line-re
...
(defvar tl--repl-buffer-p nil
  "Buffer-local flag saying if this is a Lua REPL buffer.")
(make-variable-buffer-local 'tl--repl-buffer-p)


(defadvice compilation-find-file (around tl--repl-find-file
...
(defadvice compilation-goto-locus (around tl--repl-goto-locus
                                          (msg mk end-mk)
                                          activate)
...
Usually, stdin:XX line number points to nowhere."
  (let ((errmsg-buf (marker-buffer msg))
    ...
    (if (and (with-current-buffer errmsg-buf tl--repl-buffer-p)
...
(defcustom tl-mode-hook nil
  "Hooks called when Lua mode fires up."
  :type 'hook
  :group 'tl)

(defvar tl-region-start (make-marker)
  "Start of special region for Lua communication.")

(defvar tl-region-end (make-marker)
  "End of special region for Lua communication.")

(defvar tl-emacs-menu
  '(["Restart With Whole File" tl-restart-with-whole-file t]
    ["Kill Process" tl-kill-process t]
    ["Hide Process Buffer" tl-hide-process-buffer t]
    ["Show Process Buffer" tl-show-process-buffer t]
    ["Beginning Of Proc" tl-beginning-of-proc t]
    ["End Of Proc" tl-end-of-proc t]
    ["Set Tl-Region Start" tl-set-tl-region-start t]
    ["Set Tl-Region End" tl-set-tl-region-end t]
    ["Send Tl-Region" tl-send-tl-region t]
    ["Send Current Line" tl-send-current-line t]
    ["Send Region" tl-send-region t]
    ["Send Proc" tl-send-proc t]
    ["Send Buffer" tl-send-buffer t]
    ["Search Documentation" tl-search-documentation t])
...
(eval-and-compile
  (defun tl-make-delimited-matcher (elt-regexp sep-regexp end-regexp)
    ...
    (lexical-let*
      ...
      (lambda (end)
        (let* ((prev-elt-p (match-beginning 1))
               ...
               (comment-start (tl-comment-start-pos (syntax-ppss)))
               (parse-stop end))
...
(defvar tl-font-lock-keywords
  `(;; highlight the hash-bang line "#!/foo/bar/lua" as comment
    ...
    ;; "goto label"
    (,(tl-rx (symbol (seq "goto" ws+ (group-n 1 tl-name))))
    ...
    ("^[ \t]*\\_<for\\_>"
     (,(tl-make-delimited-matcher (tl-rx tl-name) ","
                                   (tl-rx (or (symbol "in") tl-assignment-op)))
      nil nil
    ...
    ("^[ \t]*\\_<local\\_>"
     ...
     (,(tl-rx point ws tl-funcheader (* nonl))
      nil nil
...
(defvar tl-imenu-generic-expression
  `((nil ,(tl-rx (or bol ";") ws (opt (seq (symbol "local") ws)) tl-funcheader) 1))
  "Imenu generic expression for tl-mode.  See `imenu-generic-expression'.")

(defvar tl-sexp-alist '(("then" . "end")
                      ("function" . "end")
                      ("do" . "end")
                      ("interface" . "end")
                      ("repeat" . "until")))

(defvar tl-mode-abbrev-table nil
  "Abbreviation table used in tl-mode buffers.")

(define-abbrev-table 'tl-mode-abbrev-table
  '(("end"    "end"    tl-indent-line :system t)
    ("else"   "else"   tl-indent-line :system t)
    ("elseif" "elseif" tl-indent-line :system t)))
...
(defvar tl-mode-syntax-table
  (with-syntax-table (copy-syntax-table)
    ...
    (syntax-table))
  "`tl-mode' syntax table.")

;;;###autoload
(define-derived-mode tl-mode prog-mode "TypedLua"
  "Major mode for editing TypedLua code."
  :abbrev-table tl-mode-abbrev-table
  :syntax-table tl-mode-syntax-table
  :group 'tl
  (setq comint-prompt-regexp tl-prompt-regexp)
  ...
  (setq-local syntax-propertize-function
              'tl--propertize-multiline-bounds)

  (setq-local parse-sexp-lookup-properties   t)
  (setq-local indent-line-function           'tl-indent-line)
  (setq-local beginning-of-defun-function    'tl-beginning-of-proc)
  (setq-local end-of-defun-function          'tl-end-of-proc)
  (setq-local comment-start                  tl-comment-start)
  (setq-local comment-start-skip             tl-comment-start-skip)
  (setq-local comment-use-syntax             t)
  (setq-local fill-paragraph-function        #'tl--fill-paragraph)
  (with-no-warnings
  ...
  (when (boundp 'electric-indent-chars)
    ...
    (setq-local electric-indent-chars
                  (append electric-indent-chars tl--electric-indent-chars)))
  ...
  (if (and (featurep 'menubar)
           ...
           (fboundp 'add-menu)
           (not (assoc "TypedLua" current-menubar)))
      (progn
        (set-buffer-menubar (copy-sequence current-menubar))
        (add-menu nil "TypedLua" tl-emacs-menu)))
  ;; Append Lua menu to popup menu for Emacs.
  (if (boundp 'mode-popup-menu)
      (setq mode-popup-menu
            (cons (concat mode-name " Mode Commands") tl-emacs-menu)))

  ;; hideshow setup
  (unless (assq 'tl-mode hs-special-modes-alist)
    (add-to-list 'hs-special-modes-alist
                 `(tl-mode
                   ,(regexp-opt (mapcar 'car tl-sexp-alist) 'words) ;start
                   ,(regexp-opt (mapcar 'cdr tl-sexp-alist) 'words) ;end
                   nil tl-forward-sexp))))
...
(add-to-list 'auto-mode-alist '("\\.tld\\'" . tl-mode))

;;;###autoload
(add-to-list 'interpreter-mode-alist '("tl" . tl-mode))

(defun tl-electric-match (arg)
...
(defun tl--fill-paragraph (&optional justify region)
...
(defun tl-prefix-key-update-bindings ()
  (let (old-cons)
    (if (eq tl-prefix-mode-map (keymap-parent tl-mode-map))
        ;; if prefix-map is a parent, delete the parent
        (set-keymap-parent tl-mode-map nil)
      ;; otherwise, look for it among children
      (if (setq old-cons (rassoc tl-prefix-mode-map tl-mode-map))
          (delq old-cons tl-mode-map)))

    (if (null tl-prefix-key)
        (set-keymap-parent tl-mode-map tl-prefix-mode-map)
      (define-key tl-mode-map (vector tl-prefix-key) tl-prefix-mode-map))))

(defun tl-set-prefix-key (new-key-str)
  "Changes `tl-prefix-key' properly and updates keymaps

This function replaces previous prefix-key binding with a new one."
  (interactive "sNew prefix key (empty string means no key): ")
  (tl--customize-set-prefix-key 'tl-prefix-key new-key-str)
  (message "Prefix key set to %S"  (single-key-description tl-prefix-key))
  (tl-prefix-key-update-bindings))

(defun tl-string-p (&optional pos)
  "Returns true if the point is in a string."
  (save-excursion (elt (syntax-ppss pos) 3)))

(defun tl-comment-start-pos (parsing-state)
...
(defun tl-comment-or-string-p (&optional pos)
...
(defun tl-comment-or-string-start-pos (&optional pos)
...
;; 2. generic-string
;; 3. equals signs
(defconst tl-ml-begin-regexp
  "\\(?:\\(?1:-\\)-\\[\\|\\(?2:\\[\\)\\)\\(?3:=*\\)\\[")


(defun tl-try-match-multiline-end (end)
...
information provided at point and re-search-forward to it."
  (let ((comment-or-string-start-pos (tl-comment-or-string-start-pos)))
...
(defun tl-try-match-multiline-begin (limit)
  ...
  (let (last-search-matched)
    (while
        ...
        (and (setq last-search-matched
                   (re-search-forward tl-ml-begin-regexp limit 'noerror))
             ...
             (or (tl-comment-or-string-start-pos (match-beginning 0))
                 (and (eq ?- (char-after (match-beginning 0)))
                      (eq ?- (char-before (match-beginning 0)))))))

    last-search-matched))

(defun tl-match-multiline-literal-bounds (limit)
  ...
  (or (tl-try-match-multiline-end limit)
      (tl-try-match-multiline-begin limit)))

(defun tl--propertize-multiline-bounds (start end)
  "Put text properties on beginnings and ends of multiline literals.

Intended to be used as a `syntax-propertize-function'."
  (save-excursion
    (goto-char start)
    (while (tl-match-multiline-literal-bounds end)
...
Return the amount the indentation changed by."
  (let (indent
        ...
        (pos (- (point-max) (point))))
    (back-to-indentation)
    (if (tl-comment-or-string-p)
        (setq indent (tl-calculate-string-or-comment-indentation)) ;; just restore point position
      (setq indent (max 0 (tl-calculate-indentation nil))))
...
(defun tl-calculate-string-or-comment-indentation ()
  "This function should be run when point at (current-indentation) is inside string"
  (if (and (tl-string-p) (not tl-indent-string-contents))
    ...
    (let ((left-shifter-p (looking-at "\\s *\\(?:--\\)?\\]\\(?1:=*\\)\\]")))
      (save-excursion
        (goto-char (tl-comment-or-string-start-pos))
        (+ (current-indentation)
...
(defun tl-find-regexp (direction regexp &optional limit ignore-p)
...
ignored, nil otherwise."
  (let ((ignore-func (or ignore-p 'tl-comment-or-string-p))
...
(defconst tl-indentation-modifier-regexp
  ...
  (concat
   "\\(\\_<"
   (regexp-opt '("do" "function" "repeat" "then" "if" "else" "elseif" "for" "while" "interface") t)
   "\\_>\\|"
   (regexp-opt '("{" "(" "["))
   "\\)\\|\\(\\_<"
...
(defun tl-get-block-token-info (token)
  "Returns the block token info entry for TOKEN from tl-block-token-alist"
  (assoc token tl-block-token-alist))

(defun tl-get-token-match-re (token-info direction)
...
(defun tl-get-token-type (token-info)
  "Returns the relevant match regexp from token info"
   (cadddr token-info))

(defun tl-backwards-to-block-begin-or-end ()
  "Move backwards to nearest block begin or end.  Returns nil if not successful."
  (interactive)
  (tl-find-regexp 'backward tl-block-regexp))

(defun tl-find-matching-token-word (token &optional direction)
  (let* ((token-info (tl-get-block-token-info token))
         (match-type (tl-get-token-type token-info))
         ...
         (search-direction (or direction
                               (if (or (eq match-type 'open)
                                 ...
                                 'backward)
                               'backward))
         (match (tl-get-token-match-re token-info search-direction))
         maybe-found-pos)
    ...
    (catch 'found
      ...
      (if (or (and (eq search-direction 'forward)
              ...
              (and (eq search-direction 'backward)
                   (eq match-type 'open)))
          (throw 'found nil))
      (while (tl-find-regexp search-direction tl-indentation-modifier-regexp)
        ;; have we found a valid matching token?
        (let ((found-token (match-string 0))
              (found-pos (match-beginning 0)))
          (let ((found-type (tl-get-token-type
                             (tl-get-block-token-info found-token))))
            (if (not (and match (string-match match found-token)))
                ...
                (if (not (and
                          ...
                          (goto-char found-pos)
                          (tl-find-matching-token-word found-token
                                                        search-direction)))
              ...
              (setq token-info (tl-get-block-token-info token))
              (setq match (tl-get-token-match-re token-info search-direction))
              (setq match-type (tl-get-token-type token-info))))))
      maybe-found-pos)))

(defun tl-goto-matching-block-token (&optional parse-start direction)
...
the matching token if successful, nil otherwise."
  ...
  (let ((case-fold-search nil))
    (if (looking-at tl-indentation-modifier-regexp)
        (let ((position (tl-find-matching-token-word (match-string 0)
                                                      direction)))
          (and position
               (goto-char position))))))

(defun tl-goto-matching-block (&optional noreport)
...
matching keyword that ends the block, and vice versa."
  ...
  (let ((position (tl-goto-matching-block-token)))
    (if (and (not position)
             (not noreport))
        (error "Not on a block control keyword or brace")
      position)))

(defun tl-forward-line-skip-blanks (&optional back)
  ...
  (catch 'found
    (while t
      ...
      (unless (or (looking-at "\\s *\\(--.*\\)?$")
                  (tl-comment-or-string-p))
        (throw 'found (point))))))

(eval-when-compile
  (defconst tl-operator-class
    "-+*/^.=<>~:&|"))

(defconst tl-cont-eol-regexp
  (eval-when-compile
    (concat
     "\\(\\_<"
...
(defun tl-last-token-continues-p ()
  ...
  (let ((line-begin (line-beginning-position))
    ...
    (save-excursion
      ...
      ;; skip that one.
      (while (tl-find-regexp 'backward "-" line-begin 'tl-string-p)
        (if (looking-at "--")
            (setq line-end (point))))
      (goto-char line-end)
      (re-search-backward tl-cont-eol-regexp line-begin t))))

(defun tl-first-token-continues-p ()
  ...
  (let ((line-end (line-end-position)))
    (save-excursion
      ...
      ;; if strings aren't supposed to be indented, `tl-calculate-indentation' won't even let
      ;; the control inside this function
      (re-search-forward tl-cont-bol-regexp line-end t))))

(defconst tl-block-starter-regexp
  (eval-when-compile
    (concat
     "\\(\\_<"
...
(defun tl-first-token-starts-block-p ()
  "Returns true if the first token on this line is a block starter token."
  (let ((line-end (line-end-position)))
    (save-excursion
      (beginning-of-line)
      (re-search-forward (concat "\\s *" tl-block-starter-regexp) line-end t))))

(defun tl-is-continuing-statement-p (&optional parse-start)
...
"
  (let ((prev-line nil))
    (save-excursion
      (if parse-start (goto-char parse-start))
      (save-excursion (setq prev-line (tl-forward-line-skip-blanks 'back)))
      (and prev-line
           (not (tl-first-token-starts-block-p))
           (or (tl-first-token-continues-p)
               (and (goto-char prev-line)
                    ;; check last token of previous nonblank line
                    (tl-last-token-continues-p)))))))

(defun tl-make-indentation-info-pair (found-token found-pos)
  "This is a helper function to tl-calculate-indentation-info. Don't
use standalone."
  (cond
    ...
    (save-excursion
      (let ((found-bol (line-beginning-position)))
        ...
        (if (and (zerop (count-lines found-bol (line-beginning-position)))
                 (not (looking-at tl-indentation-modifier-regexp)))
   ...
   ;; tl-calculate-indentation-override.
   ...
   ((member found-token (list "until" "elseif"))
    (save-excursion
      (let ((line (line-number-at-pos)))
        (if (and (tl-goto-matching-block-token found-pos 'backward)
   ...
   ((string-equal found-token "else")
     (save-excursion
       (let ((line (line-number-at-pos)))
         (if (and (tl-goto-matching-block-token found-pos 'backward)
                  (= line (line-number-at-pos)))
             (cons 'replace-matching (cons 'relative tl-indent-level))
                   (cons 'relative tl-indent-level)))))
   ...
   ((member found-token (list ")" "}" "]" "end"))
    (save-excursion
      (let ((line (line-number-at-pos)))
        (tl-goto-matching-block-token found-pos 'backward)
        (if (/= line (line-number-at-pos))
            (tl-calculate-indentation-info (point))
          (cons 'remove-matching 0)))))
...
(defun  tl-add-indentation-info-pair (pair info)
...
(defun tl-calculate-indentation-info-1 (indentation-info bound)
  "Helper function for `tl-calculate-indentation-info'.

Return list of indentation modifiers from point to BOUND."
  (while (tl-find-regexp 'forward tl-indentation-modifier-regexp
                          bound)
    (let ((found-token (match-string 0))
          (found-pos (match-beginning 0))
      ...
      (setq indentation-info
            (tl-add-indentation-info-pair
             (tl-make-indentation-info-pair found-token found-pos)
             indentation-info))))
  indentation-info)


(defun tl-calculate-indentation-info (&optional parse-end)
...
and relative each, and the shift/column to indent to."
  (let ((combined-line-end (line-end-position))
    ...
    (while (tl-is-continuing-statement-p)
      (tl-forward-line-skip-blanks 'back))
    ...
    (back-to-indentation)
    (setq indentation-info
          (tl-calculate-indentation-info-1
...
(defun tl-accumulate-indentation-info (info)
  "Accumulates the indentation information previously calculated by
tl-calculate-indentation-info. Returns either the relative indentation
...
(defun tl-calculate-indentation-block-modifier (&optional parse-end)
  "Return amount by which this line modifies the indentation.
Beginnings of blocks add tl-indent-level once each, and endings
of blocks subtract tl-indent-level once each. This function is used
...
one."
  (let (indentation-info)
    (save-excursion
      ...
      ;; tl-calculate-indentation-info will scan through the whole thing
      (let ((case-fold-search nil))
        (setq indentation-info
              (tl-accumulate-indentation-info
               (tl-calculate-indentation-info parse-end)))))
...
(defconst tl--left-shifter-regexp
  (eval-when-compile
    (rx
     ...
     (or (seq (group-n 1 symbol-start "local" (+ blank)) "function" symbol-end)
         ...
         (seq (group-n 1 (eval tl--function-name-rx) (* blank)) (any "{("))
         (seq (group-n 1 (or
              ...
              (or "{"
                  "function"
                  (seq (group-n 1 (eval tl--function-name-rx) (* blank))
                       (any "({")))))))
...
(defun tl-point-is-after-left-shifter-p ()
  "Check if point is right after a left-shifter expression.

See `tl--left-shifter-regexp' for description & example of
left-shifter expression. "
  (save-excursion
    (let ((old-point (point)))
      ...
      (and
       (/= (point) old-point)
       (looking-at tl--left-shifter-regexp)
       (= old-point (match-end 1))))))



(defun tl-calculate-indentation-override (&optional parse-start)
...
If not, return nil."
  (let (case-fold-search token-info block-token-pos)
    (save-excursion
      ...
      (back-to-indentation)
      (unless (tl-comment-or-string-p)
        (while
            (and (looking-at tl-indentation-modifier-regexp)
                 (setq token-info (tl-get-block-token-info (match-string 0)))
                 (not (eq 'open (tl-get-token-type token-info))))
          (setq block-token-pos (match-beginning 0))
          (goto-char (match-end 0))
          (skip-syntax-forward " " (line-end-position)))

        (when (tl-goto-matching-block-token block-token-pos 'backward)
          ...
          (if (tl-point-is-after-left-shifter-p)
              (current-indentation)
            (current-column)))))))

(defun tl-calculate-indentation (&optional parse-start)
  "Return appropriate indentation for current line as Lua code."
  (save-excursion
    (let ((continuing-p (tl-is-continuing-statement-p))
...
Returns t unless search stops due to beginning or end of buffer."
  ...
  (while (and (> arg 0)
              (re-search-backward tl--beginning-of-defun-re nil t))
    (setq arg (1- arg)))

  (while (and (< arg 0)
              (re-search-forward tl--beginning-of-defun-re nil t))
    (beginning-of-line)
    (setq arg (1+ arg)))

  (zerop arg))

(defun tl-end-of-proc (&optional arg)
...
;;;###autoload
(defalias 'run-lua #'tl-start-process)

;;;###autoload
(defun tl-start-process (&optional name program startfile &rest switches)
  "Start a Lua process named NAME, running PROGRAM.
PROGRAM defaults to NAME, which defaults to `tl-default-application'.
When called interactively, switch to the process buffer."
  (interactive)
  (or switches
      (setq switches tl-default-command-switches))
  (setq name (or name (if (consp tl-default-application)
                          (car tl-default-application)
                        tl-default-application)))
  (setq program (or program tl-default-application))
  (setq tl-process-buffer (apply 'make-comint name program startfile switches))
  (setq tl-process (get-buffer-process tl-process-buffer))
  (set-process-query-on-exit-flag tl-process nil)
  (with-current-buffer tl-process-buffer
    ;; wait for prompt
    (while (not (tl-prompt-line))
      (accept-process-output (get-buffer-process (current-buffer)))
      (goto-char (point-max)))
    ;; send initialization code
    (tl-send-string tl-process-init-code)

    ;; enable error highlighting in stack traces
    (require 'compile)
    (setq tl--repl-buffer-p t)
    (make-local-variable 'compilation-error-regexp-alist)
    (setq compilation-error-regexp-alist
          (cons (list tl-traceback-line-re 1 2)
                compilation-error-regexp-alist))
    (compilation-shell-minor-mode 1))

  ;; when called interactively, switch to process buffer
  (if (called-interactively-p 'any)
      (switch-to-buffer tl-process-buffer)))

(defun tl-get-create-process ()
  "Return active Lua process creating one if necessary."
  (unless (comint-check-proc tl-process-buffer)
    (tl-start-process))
  tl-process)

(defun tl-kill-process ()
  "Kill Lua process and its buffer."
  (interactive)
  (when (buffer-live-p tl-process-buffer)
    (kill-buffer tl-process-buffer)
    (setq tl-process-buffer nil)))

(defun tl-set-tl-region-start (&optional arg)
  "Set start of region for use with `tl-send-tl-region'."
  (interactive)
  (set-marker tl-region-start (or arg (point))))

(defun tl-set-tl-region-end (&optional arg)
  "Set end of region for use with `tl-send-tl-region'."
  (interactive)
  (set-marker tl-region-end (or arg (point))))

(defun tl-send-string (str)
  "Send STR plus a newline to the Lua process.

If `tl-process' is nil or dead, start a new process first."
  (unless (string-equal (substring str -1) "\n")
    (setq str (concat str "\n")))
  (process-send-string (tl-get-create-process) str))

(defun tl-send-current-line ()
  "Send current line to the Lua process, found in `tl-process'.
If `tl-process' is nil or dead, start a new process first."
  (interactive)
  (tl-send-region (line-beginning-position) (line-end-position)))

(defun tl-send-defun (pos)
...
(defun tl-send-region (start end)
  (interactive "r")
  (setq start (tl-maybe-skip-shebang-line start))
  (let* ((lineno (line-number-at-pos start))
         (tl-file (or (buffer-file-name) (buffer-name)))
         ...
         (command
          ...
          (format "print(''); luamode_loadstring(%s, %s, %s);\n"
                  (tl-make-tl-string region-str)
                  (tl-make-tl-string tl-file)
                  lineno)))
    (tl-send-string command)
    (when tl-always-show (tl-show-process-buffer))))

(defun tl-prompt-line ()
  (save-excursion
    (save-match-data
      (forward-line 0)
...
(defun tl-send-tl-region ()
  "Send preset Lua region to Lua process."
  (interactive)
  (unless (and tl-region-start tl-region-end)
    (error "tl-region not set"))
  (tl-send-region tl-region-start tl-region-end))

(defalias 'tl-send-proc 'tl-send-defun)

(defun tl-send-buffer ()
  "Send whole buffer to Lua process."
  (interactive)
  (tl-send-region (point-min) (point-max)))

(defun tl-restart-with-whole-file ()
  "Restart Lua process and send whole file as input."
  (interactive)
  (tl-kill-process)
  (tl-send-buffer))

(defun tl-show-process-buffer ()
  "Make sure `tl-process-buffer' is being displayed.
Create a Lua process if one doesn't already exist."
  (interactive)
  (display-buffer (process-buffer (tl-get-create-process))))


(defun tl-hide-process-buffer ()
  "Delete all windows that display `tl-process-buffer'."
  (interactive)
  (when (buffer-live-p tl-process-buffer)
    (delete-windows-on tl-process-buffer)))

(defun tl-funcname-at-point ()
...
(defun tl-search-documentation ()
  "Search Lua documentation for the word at the point."
  (interactive)
  (let ((url (concat tl-documentation-url "#pdf-" (tl-funcname-at-point))))
    (funcall tl-documentation-function url)))

(defun tl-toggle-electric-state (&optional arg)
...
left out."
  ...
  (let ((num_arg (prefix-numeric-value arg)))
    (setq tl-electric-flag (cond ((or (null arg)
                                       (zerop num_arg)) (not tl-electric-flag))
                                  ((< num_arg 0) nil)
                                  ((> num_arg 0) t))))
  (message "%S" tl-electric-flag))

(defun tl-forward-sexp (&optional count)
  ...
  (save-match-data
    (let* ((count (or count 1))
           (block-start (mapcar 'car tl-sexp-alist))
           (block-end (mapcar 'cdr tl-sexp-alist))
      ...
      (while (> count 0)
        ...
        (if (looking-at (regexp-opt block-start 'words))
            (let ((keyword (match-string 1)))
              (tl-find-matching-token-word keyword 'forward))
...
(define-key tl-mode-menu [restart-with-whole-file]
  '("Restart With Whole File" .  tl-restart-with-whole-file))
(define-key tl-mode-menu [kill-process]
  '("Kill Process" . tl-kill-process))

(define-key tl-mode-menu [hide-process-buffer]
  '("Hide Process Buffer" . tl-hide-process-buffer))
(define-key tl-mode-menu [show-process-buffer]
  '("Show Process Buffer" . tl-show-process-buffer))

(define-key tl-mode-menu [end-of-proc]
  '("End Of Proc" . tl-end-of-proc))
(define-key tl-mode-menu [beginning-of-proc]
  '("Beginning Of Proc" . tl-beginning-of-proc))

(define-key tl-mode-menu [send-tl-region]
  '("Send Tl-Region" . tl-send-tl-region))
(define-key tl-mode-menu [set-tl-region-end]
  '("Set Tl-Region End" . tl-set-tl-region-end))
(define-key tl-mode-menu [set-tl-region-start]
  '("Set Tl-Region Start" . tl-set-tl-region-start))

(define-key tl-mode-menu [send-current-line]
  '("Send Current Line" . tl-send-current-line))
(define-key tl-mode-menu [send-region]
  '("Send Region" . tl-send-region))
(define-key tl-mode-menu [send-proc]
  '("Send Proc" . tl-send-proc))
(define-key tl-mode-menu [send-buffer]
  '("Send Buffer" . tl-send-buffer))
(define-key tl-mode-menu [search-documentation]
  '("Search Documentation" . tl-search-documentation))
